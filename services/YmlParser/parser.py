    ...
    for elem in root.findall("shop"):
        ...
        if delivery != "false" and pickup != "false":
            if len(arrdel) != 0 and len(arrpick) != 0:
                ...
                data["pickup"] = arrpick
            elif (delivery != "false" or len(arrdel) != 0) and (
                    pickup == "false" or pickup == 'None' or len(arrpick) != 0):
                data["delivery"] = arrdel
                if pickup == 'None':
                    data["pickup"] = "false"
                else:
                    data["pickup"] = pickup
            elif (delivery == "false" or delivery == 'None' or len(arrdel) != 0) and (
                    pickup != "false" or len(arrpick) != 0):
                if delivery == 'None':
                    data["delivery"] = "false"
                else:
                    data["pickup"] = arrpick
